...
const EmployeeTable = () => {
  ...
  // Function to handle sorting by Join Date
  const handleSort = (e) => {
    const value = e.target.value;

    if (value === "sequential") {
      let sortedData = [...SearchData].sort((a, b) => {
        let dateA = new Date(a.joinDate.split("/").reverse().join("-"));
        let dateB = new Date(b.joinDate.split("/").reverse().join("-"));
        return dateA - dateB; // Sort in ascending order
      });

      setSearchData(sortedData);
    } else {
      setSearchData(emp_details); // Reset if another option is selected
    }
  };
        ...
        <table className="styled-table">
            ...
            <tr>
              ...
              <th>
                <select onChange={handleSort}>
                  ...
                  <option value="sequential">Sequential Join Date</option>
                </select>
              </th>
              <th>
                <select>
          ...
          <tbody>
            {SearchData.map((e, index) => (
              <tr key={index}>
                <td>
                  <input type="radio" name="employee" /> {e.siNo}
                </td>
                <td>
                  <span style={css}>{e.name}</span> <br />
                  <span style={{ color: "#2F2F2F66" }}>{e.name + "123@gmail.com"}</span>
                </td>
